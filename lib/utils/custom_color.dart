import 'package:flutter/material.dart';

class AppColor {
  static Color paleBlue = const Color(0xffa9b8ec);
  static Color blue = const Color(0xff2a2b90);
  static Color deepBlue = const Color(0xff8d9bcb);
  static Color altDeepBlue = const Color(0xff7c88b7);
  static Color inputColor = const Color(0xfff3f3f4);
  static Color label = const Color(0xFF607D8B);
  static Color disable = const Color(0xFFD4D4D4);
  static Color gold = const Color(0xFFFFA600);
  static Color green = const Color(0xFF28D811);
  static Color purple = const Color(0xFFC111D8);

  static Color red = const Color(0xFFF20F0F);
  static Color textBlack = const Color(0x61000000);
}
